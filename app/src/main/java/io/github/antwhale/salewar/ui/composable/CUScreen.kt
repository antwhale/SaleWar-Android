...
fun CUScreen(modifier: Modifier, cuViewModel: CUViewModel) {
    ...
    val showingFavoriteList by cuViewModel.showingFavoriteList.collectAsState()
    val favoriteProducts by cuViewModel.favoriteProducts.collectAsState()
        ...
        Column(horizontalAlignment = Alignment.CenterHorizontally) {
            ...
            SaleWarTitleBar(
                Modifier
                    .fillMaxWidth()
                    .wrapContentHeight(),
                onClickFavoriteMenu = { cuViewModel.showingFavoriteList.value = true}
            )
            ...
            if(showingFavoriteList){
                FavoriteProductList(
                    ...
                    onDeleteFavoriteProduct = { product -> cuViewModel.deleteFavoriteProduct(Product(img = product.img, title = product.title, price = product.price, saleFlag = product.saleFlag, store = product.store))},
                    onDismiss = { cuViewModel.showingFavoriteList.value = false }
                )
            }
        }
    }
